    ...
    if call_limit:
        if FLAG == 0:
            ...
            if len(horizontal_coor) >= 2:
                ...
                horizontal_right = sorted(horizontal_coor, key=lambda i: i[0] + i[2])
                if abs(horizontal_coor[1][0] - horizontal_coor[0][0]) / (horizontal_coor[1][0] + epsilon) > gap_rate:
                    x, y, w, h = horizontal_coor[0]
                    src[y:y+h, x:x+w] = 0

                elif abs(horizontal_right[-1][0] + horizontal_right[-1][2] - (horizontal_right[-2][0] + horizontal_right[-1][2])) / \
            ...
            if len(vertical_coor) >= 2:
                ...
                vertical_down = sorted(vertical_coor, key=lambda i: i[1] + i[3])
                if abs(vertical_coor[1][1] - vertical_coor[0][1]) / (vertical_coor[1][1] + epsilon) > gap_rate:
                    x, y, w, h = vertical_coor[0]
                    src[y:y+h, x:x+w] = 0

                elif abs(vertical_down[-1][1] + vertical_down[-1][3] - (vertical_down[-2][1] + vertical_down[-2][3])) / \
...
def table_lines(ori_src, shape):
    ...
    src = cv2.imdecode(img_np_arr, cv2.IMREAD_COLOR)
    src = 255 - src
    Image.fromarray(src).save('unet_table.jpg')
    print(11111, src.shape)
    ...
    # # 去除多余线段
    # horizontal = remove_line(horizontal, FLAG=0, gap_rate=0.1)
    # vertical = remove_line(vertical, FLAG=1, gap_rate=0.1)
    ...
    # print(horizontal.shape, vertical.shape)
    joints = cv2.bitwise_and(horizontal, vertical)
    Image.fromarray(joints).save('joints.jpg')
    # x, y, w, h = cv2.boundingRect(mask)
    # Image.fromarray(mask[y:y+h, x:x+w]).show()
    # cv2.rectangle(mask, (x, y), (x + w - 1, y + h - 1), (255, 255, 255), 1)
    mask = cv2.blur(mask, (2, 2))
...
def generate_table(cell, src):
        ...
        for col in range(col_begin, col_end):
            for row in range(row_begin, row_end):
                for cell in cell_list:
                    if cell[2]['col_begin'] == col and cell[2]['col_end'] == col+1 and\
                            cell[2]['row_begin'] == row and cell[2]['row_end'] == row+1:
                        cell[-1] = 1
    # 没有检测到单元格则赋值
    for i in cell_list:
        if i[-1] == 0:
    ...
    for index_i, i in enumerate(tables):
        ...
        str_location = '_'.join([str(v) for v in i[-1].values()])
        cell_region = [i[1][0], i[1][1], i[1][0] + i[1][2], i[1][1] + i[1][3]]
        import random
        color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
        cv2.putText(save_table, str_location, (cell_region[0]+int(i[1][2]/2), cell_region[1]+int(i[1][3]/2)), cv2.FONT_HERSHEY_COMPLEX, 0.5, color, 1)
        cv2.rectangle(save_table, (cell_region[0], cell_region[1]), (cell_region[2], cell_region[3]), color, 1)
        word_str = []
