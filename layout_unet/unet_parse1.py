...
class unetParse(object):
    def __init__(self, image, line_infos, document):
        ...
        self.lines = []  # 元素格式示例 ['一二三', 10.5, 2353, 'footer_center'] [文字， 字体大小， 纵坐标， 居中格式]
    ...
    def unetpic2typepos(self):
        ...
        res = requests.post('http://172.30.20.154:32081/layout', data={'image': pic_str})
        # print(res.content)
        # return res.json()
        # img 为unet版式检测的RGB结果图
        ...
        # imgH = img.shape[0]
        # img_dict = {}
        # img_dict['text'] = np.where(img==COLOR_DICT[1][0], COLOR_DICT[1][0], -1).astype(np.uint8)
        # img_dict['table'] = np.where(img==COLOR_DICT[2][0], COLOR_DICT[2][0], -1).astype(np.uint8)
        # img_dict['header'] = np.where(img==COLOR_DICT[3][0], COLOR_DICT[3][0], -1).astype(np.uint8)
        # img_dict['footer'] = np.where(img==COLOR_DICT[4][0], COLOR_DICT[4][0], -1).astype(np.uint8)
        # img_dict['figure'] = np.where(img==COLOR_DICT[5][0], COLOR_DICT[5][0], -1).astype(np.uint8)
        # img_dict = res.json()
        # im_temp = self.image
        # for key in img_dict.keys():
        #     for li in img_dict[key]:
        #         x, y, w, h = li[0]
        #         cv2.rectangle(im_temp, pt1=(x, y), pt2=(x+w, y+h),color=(255, 0, 0), thickness=1)
        # Image.fromarray(im_temp).save('layout_unet/result.jpg')
        #         # 剔除部分错误的页眉页脚
        #         if key == 'footer':
        #             if y < 0.8*imgH:
        #                 continue
        ...
        #     bboxs = sorted(bboxs, key=lambda x: x[0][1])
        #     img_dict[key] = bboxs
        return res.json()
        # return img_dict
    ...
    def layout(self, texts, texts_w, line_type):
        ...
        for i in new_texts:
            ...
            if i[-1] == 'sub':
                ...
                text_tmp = ['', 0]
        if text_tmp[0] != '':
            text_tmp[1] = self.get_font(new_texts[-1][0][0])
            text_tmp.extend([new_texts[-1][0][0][1], line_type])
            self.lines.append(text_tmp)
        #         end_flg = '\n'
        #
    ...
    def filltexts(self):
        ...
        for table in tables:
            ...
            try:
                ...
                pic_str = base64.b64encode(buffer)
                pic_str = pic_str.decode()
                r = requests.post('http://172.30.20.154:32082/table_line', data={"img": pic_str})
    ...
    def save2docx(self):
        ...
        for i in res:
            ...
            if 'header' in i[-1]:
                # header_pg = header.paragraphs[-1].text
                if header.paragraphs[-1].text != '':
                    header.add_paragraph()
                header.paragraphs[-1].style.font.size = Pt(i[1]-2)
                header.paragraphs[-1].text += i[0]
                header.paragraphs[-1].paragraph_format.space_after = Pt(0)
                header.paragraphs[-1].paragraph_format.line_spacing_rule = 0
                if '_' in i[-1]:
                    header.paragraphs[-1].alignment = paragraph_type_dict[i[-1].split('_')[1]]
                continue
            if'footer' in i[-1]:
                if footer.paragraphs[-1].text != '':
                    footer.add_paragraph()
                footer.paragraphs[-1].style.font.size = Pt(i[1] - 2)
                footer.paragraphs[-1].text += i[0]
                footer.paragraphs[-1].paragraph_format.space_after = Pt(0)
                footer.paragraphs[-1].paragraph_format.line_spacing_rule = 0
                if '_' in i[-1]:
