...
def xy_rotate_box(cx,cy,w,h,angle):        
    ...
    h     = float(h)
    angle = float(angle)
    x1,y1 = rotate(cx-w/2,cy-h/2,angle,cx,cy) # left top
    x2,y2 = rotate(cx+w/2,cy-h/2,angle,cx,cy) # right top
    x3,y3 = rotate(cx+w/2,cy+h/2,angle,cx,cy) # right bottom
    x4,y4 = rotate(cx-w/2,cy+h/2,angle,cx,cy) # left bottom
    return x1,y1,x2,y2,x3,y3,x4,y4
